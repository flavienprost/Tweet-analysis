...
classif_predictions=pickle.load(open(current_dir+ '/Predictions_Classifier/Log'+ "_predictions_val_N_feat="+ str(N_feat) + "_N_sentences="+ str(N_sentence)+ ".p", "rb"))

print('Creating the table')
for i in range(0,20000):#length of the validation set
    features_train_stacking.append([classif_predictions[i]])

for clf in Classifier:
    if clf<>'Log':

        classif_predictions=pickle.load(open(current_dir+ '/Predictions_Classifier/'+clf+ "_predictions_val_N_feat="+ str(N_feat) + "_N_sentences="+ str(N_sentence) + ".p", "rb"))
